...
\end{subfigure} 
	\end{minipage}
	\caption{Reconstruction of the optic radiation (OR) Tract. To make the comparison
	between the reference and the tractographies more intuitive, we have
	indicated contour of the reference in each reconstruction with a black
	line. For the low rank approximation it is visible that the consensus
	model reduces the false positive streamlines.  The consensus average
	model has a much higher overlap compared to the base average model and
	the consensus selection model also increases the overlap. All
reconstructions have problems to follow the tract in the upper region where we
have an high curvature.  }
	\label{fig:OR}
\end{figure*}
